...
function _train(;epochs = 45, batchsize = 1000, device = gpu)
    ...
    for epoch in 1:epochs
        @showprogress "training epoch $epoch/$epochs" for (x, y) in train_loader .|> device
        ...
        plt = scatterplot(1:epoch, train_loss_hist, name = "train_loss", xlabel="epoch", ylim=[0,3])
        scatterplot!(plt, 1:epoch, train_acc_hist, name = "train_acc")
        scatterplot!(plt, 1:epoch, test_loss_hist, name = "test_loss")
        scatterplot!(plt, 1:epoch, test_acc_hist, name = "test_acc")
        display(plt)
        if device === gpu
            CUDA.memory_status()
